...
Response Register::reg(Request req)
{
	...
	else if ("POST" == method)
	{
		...
		if(!reg_user.match(m_user))
		{
			...
			data = FileUtil::loadFile(HTML_PATH + "userError.html");
			res.setResData(data);
			return res;
		}
		...
		if(!reg_passwd.match(m_passwd))
		{
			...
			data = FileUtil::loadFile(HTML_PATH + "passwdError.html");
			res.setResData(data);
			return res;
		}
		...
		{
			data = FileUtil::loadFile(HTML_PATH + "registerError.html");
		}
