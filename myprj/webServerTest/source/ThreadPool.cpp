...
CREATE_STATIC_LOGGERMANAGER(ThreadPool);

int ThreadPool::threadpool_create(int threadCount, int queueSize)
{
    LogDebug("ThreadPool::threadpool_create threadCount:[%d],queueSize:[%d]", threadCount, queueSize);
    bool err = false;
    do
    {
...
void *ThreadPool::threadpool_thread(void *args)
{
    ...
    --started;
    pthread_mutex_unlock(&lock);
    LogDebug("This threadpool thread finishs!\n");
    pthread_exit(NULL);
    return (NULL);
}